        	...
        	<?php
				...
				foreach($posts as $p) {?>
                    <li>
                        ...
                        <?php if ($instance['display_featured_image'] && has_post_thumbnail($p->ID) ) { ?><?php echo get_the_post_thumbnail($p->ID,array(90,90), array()); ?> <?php } ?>
                        <?php if ( $instance['display_title'] ) { ?> <a href="<?php echo get_permalink($p->ID); ?>" class="title" rel="bookmark" title="<?php echo $p->post_title; ?>"><?php echo $p->post_title?></a><?php } ?>
						...
						?>
						<span class='date'><?php echo get_the_time('F j,Y',$p->ID);?></span>
						<?php } else $class='';?>
                    </li>
                <?php
                }
